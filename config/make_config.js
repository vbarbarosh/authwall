...
function make_config(input = {})
{
    ...
    if (config.flows.twitter.enabled) {
        const {client_id, client_secret, redirect_url} = config.flows.twitter;
        if (!client_id || !client_secret || !redirect_url) {
            config.flows.twitter.enabled = false;
            if (client_id || client_secret || redirect_url) {
                console.warn('⚠️  X OAuth disabled: client_id, client_secret, and redirect_url must all be set');
            }
        }
    }
...
function secret_hkdf(secret, namespace)
{
    return Buffer.from(crypto.hkdfSync('sha256', secret, 'authwall', namespace, 32)).toString('base64url');
}

function load_secret(env)
{
    if (env.AUTHWALL_SECRET) {
        validate_secret(env.AUTHWALL_SECRET, 'AUTHWALL_SECRET');
        return env.AUTHWALL_SECRET;
    }
...
function get_knexvars(env)
{
    const db = env.AUTHWALL_DB;
    if (!db) {
