...
describe('POST /auth/sessions/revoke', function () {
    ...
    it('revokes another session', async function () {
        ...
        await this.http_post_json('/auth/sessions/revoke', {uid: session_b.uid, _csrf: status.csrf_token});
    ...
    it('fails with missing uid', async function () {
        await this.sign_in({username: 'mocha', password: 'pass123'});
        const status = await this.http_get_json('/auth/status');
        await this.http_post_json('/auth/sessions/revoke', {_csrf: status.csrf_token});
        const status2 = await this.http_get_json('/auth/status');
    ...
    it('requires authentication', async function () {
        const status = await this.http_get_json('/auth/status');
        await this.http_post_json('/auth/sessions/revoke', {uid: 'some-uid', _csrf: status.csrf_token});
        const status2 = await this.http_get_json('/auth/status');
