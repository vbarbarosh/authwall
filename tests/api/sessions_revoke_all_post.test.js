...
describe('POST /auth/sessions/revoke-all', function () {
    ...
    it('revokes all other sessions', async function () {
        ...
        const s_a = await this.client.get_json('/auth/status');
        ...
        const s_b = await this.client.get_json('/auth/status');
        ...
        await this.client.post_json('/auth/sessions/revoke-all', {_csrf: status.csrf_token});

        // Client A still authenticated
        const status_a = await this.client.get_json('/auth/status');
        assert.strictEqual(status_a.authenticated, true);
        assert.strictEqual(status_a.sessions.length, 1);

        // Client B is now invalid
        this.client.cookies = cookies_b;
        const status_b = await this.client.get_json('/auth/status');
