...
describe('Brute-force protection', function () {
    ...
    it('blocks magic link confirm after 3 wrong code attempts', async function () {
        ...
        for (let i = 0; i < 3; i++) {
            await this.http_post_json('/auth/magic-link/confirm', {email: 'mocha@authwall.test', code: '000000', _csrf: status.csrf_token});
        }

        // correct code should now be rejected too
        const {code} = this.sent_emails[0].placeholders;
        await this.http_post_json('/auth/magic-link/confirm', {email: 'mocha@authwall.test', code, _csrf: status.csrf_token});
